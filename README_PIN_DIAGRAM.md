...
| **Left Switch (Key 1)** | Signal | `GPIO 25` | Digital IO | — | Direct to GND (Internal `INPUT_PULLUP`). Dedicated Page Navigation (Forward, Backward & Home). |
| **Right Switch (Key 2)** | Signal | `GPIO 26` | Digital IO | — | Direct to GND (Internal `INPUT_PULLUP`). Page-Specific Functionality (Timer toggle, Task checkoff, Refresh, Theme cycle). |
