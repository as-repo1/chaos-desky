...
public:
    ...
    bool begin() {
        ...
        tft.setRotation(TFT_ROTATION); // 180 degree rotation (rotation 2)
    ...
    void renderCurrentPage(const OutdoorWeatherData& weather, 
        ...
        switch (currentPage) {
            case 0:
                renderOutdoorWeatherPage(weather);
                break;
            case 1:
                renderPressureGraphPage(sensors);
                break;
            case 2:
                renderPomodoroPage(pomo, timeStr);
                break;
            case 3:
                renderSystemQrPage(ipStr);
                break;
        }
    }
...
private:
    ...
    void renderOutdoorWeatherPage(const OutdoorWeatherData& w) {
        tft.setTextColor(COLOR_TEXT);
        tft.setCursor(4, 22);
        tft.setTextSize(1);
        tft.printf("LOCATION: %s", w.cityName.c_str());
        ...
        tft.setCursor(10, 36);
        if (w.valid) {
            tft.printf("%.0fC", w.tempC);
        } else {
            tft.print("--C");
        }

        // Draw Weather Graphic Icon (Simulated graphics)
        ...
        tft.setTextColor(COLOR_PRIMARY);
        tft.setCursor(4, 68);
        tft.printf("STATUS: %s", w.condition.c_str());
        ...
        tft.setTextColor(COLOR_TEXT);
        tft.setCursor(4, 88);
        tft.printf("HUMIDITY: %.0f%%", w.humidity);

        tft.setCursor(4, 102);
        tft.printf("TEMP MIN: %.1fC", w.tempMinC);

        tft.setCursor(4, 116);
        tft.printf("TEMP MAX: %.1fC", w.tempMaxC);
    ...
    void renderPressureGraphPage(SensorManager& sm) {
        tft.setTextColor(COLOR_TEXT);
        tft.setTextSize(1);
        tft.setCursor(4, 20);
        tft.printf("PRESS: %.1f hPa", sm.data.pressureHpa);
        ...
        // 24h Pressure Graph Frame (x: 4 to 124, y: 60 to 120)
        ...
        tft.setTextColor(COLOR_BG);
        tft.setCursor(4, 149);
        tft.printf("TREND: %+.1fhPa/6h", trendVal);
    }

    // --- Page 3: Cyberpunk Pomodoro Hub ---
    void renderPomodoroPage(PomodoroTimer& pomo, const String& timeStr) {
        ...
        // Progress Bar (Box: x: 10, y: 98, w: 108, h: 14)
        ...
        tft.printf("SESSIONS DONE: %d", pomo.completedSessions);
    ...
    void renderSystemQrPage(const String& ipStr) {
        tft.setTextColor(COLOR_TEXT);
        tft.setTextSize(1);
        tft.setCursor(4, 20);
        tft.printf("IP: %s", ipStr.c_str());

        // Render QR Code (Targeting http://<ESP32_IP>)
        if (ipStr.length() > 7 && ipStr != "0.0.0.0") {
            String qrUrl = "http://" + ipStr;
            QRCode qrcode;
            uint8_t qrcodeData[qrcode_getBufferSize(3)];
            qrcode_initText(&qrcode, qrcodeData, 3, 0, qrUrl.c_str());

            // QR Code bounding box: size = 29x29 modules. Scale by 3 = 87x87 px
            int scale = 3;
        ...
        // Memory Stats
        tft.setCursor(4, 128);
        tft.printf("RAM FREE: %u KB", ESP.getFreeHeap() / 1024);
